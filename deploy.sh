...
if [[ ! -n "$NEXT_MANIFEST_PATH" ]]; then
  NEXT_MANIFEST_PATH=$KUDU_SYNC_CMD
fi
...
if [[ "$IN_PLACE_DEPLOYMENT" -ne "1" ]]; then
  "$KUDU_SYNC_CMD" -v 50 -f "$DEPLOYMENT_SOURCE" -t "$DEPLOYMENT_TARGET" -n "$NEXT_MANIFEST_PATH" -p "$ARTIFACTS/manifest" -i ".git;.hg;.deployment;deploy.sh"
  exitWithMessageOnError "Kudu Sync failed"
fi

# 2. Select node version
if [[ -n "$KUDU_SELECT_NODE_VERSION_CMD" ]]; then
  SELECT_NODE_VERSION="$KUDU_SELECT_NODE_VERSION_CMD \"$DEPLOYMENT_TARGET\" \"$DEPLOYMENT_SOURCE\" \"$ARTIFACTS\" \"$DEPLOYMENT_TEMP\""
